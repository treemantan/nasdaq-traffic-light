...
REM 2) optionally download IBKR Flex data from secrets\local_pipeline.env
REM 3) import all CSV/XML statements into portfolio.csv
REM 4) generate the market report locally without sending email
REM
REM Optional local env file uses the same names as GitHub Actions:
REM   secrets\local_pipeline.env
REM   IBKR_FLEX_TOKEN
