...
ETF 模块还会用 Yahoo 5 年日线做轻量历史检验。回测采用周度抽样，默认把 `新增仓位环境 >= 60` 视为“质量不差”的环境样本，并观察之后约 1M/3M/6M 的 forward return、3M 胜率和3M最大回撤，再与全样本对比。除此之外，系统会用当前的分数、RSI、1M/3M动量、距50/200日线、`σ200` 和日波动率做相似度匹配，寻找历史上最接近当前环境的样本，并展示这些相似样本之后的 1M/3M/6M 表现与3M回撤。二级校准层会同时检验 `>=60`、`>=70`、`>=75` 三个阈值：`>=60` 表示环境尚可，`>=70` 表示环境较好，`>=75` 表示趋势结构较强但需检查拥挤度；系统会根据 forward return、3M胜率、3M回撤、样本数和相对全样本优势，动态给出“历史最优阈值”或“未发现稳定阈值优势”。该检验只使用当时已经可见的价格趋势、动量和波动信息，不使用未来数据，也不把当前宏观指标回填到历史；因此它是环境评分的历史解释力检查，不是交易策略回测或买卖信号。
