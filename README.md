...
相似度匹配不使用未来收益、未来回撤或事后新闻事件作为输入。对于每只 ETF，系统先用当前时点和每个历史周度时点当时已经可见的特征计算加权标准化距离：
...
- `distance = 0` 表示输入特征完全一致。
- 数值越低，代表历史起点与当前环境越接近。
- 距离不是概率，也没有机械的“低于某个数就一定会重演”阈值。
- 如果某项历史数据不可用，该特征会从该次距离计算中剔除，不会用虚构值补齐。
- 每个历史候选样本必须达到至少 `65%` 的有效特征覆盖率，否则不会进入相似样本池。页面会展示代表样本平均覆盖率和“历史可比性”等级。
- 系统按距离从低到高选择最接近的12个历史周度样本，再观察这些样本之后的路径。

实现上，历史 ETF 特征严格使用 `history[: historical_index + 1]`；跨资产历史特征也只截取到该历史日期或该日期之前最近的有效交易日。未来 1M/3M/6M 收益和3M回撤在距离排序完成后才作为 outcome 读取。尾部案例标签和事件线索同样只在匹配完成后生成。因此，系统做的是“先按当时已知信息匹配，再查看后来发生了什么”，而不是“先找到后来下跌的日期，再反向挑选看起来相似的变量”。

距离同时覆盖 ETF 自身结构和跨资产环境。`scale` 用于把不同单位转换为可比较的偏离幅度；`weight` 用于提高关键变量的解释权重。表中的 scale 是保护性基准值。实际运行会基于该 ETF 历史特征分布使用 MAD（中位绝对偏差）自适应校准，并限制在基准值的 `0.5x` 至 `2x` 区间，避免高波动主题 ETF 与宽基 ETF 被迫使用完全相同的尺度，也避免少量异常值让尺度失真：

| 特征组 | 特征 | scale | weight |
| --- | --- | ---: | ---: |
| ETF自身 | 新增仓位环境分数 | 15 | 1.2 |
| ETF自身 | RSI14 | 15 | 1.0 |
| ETF自身 | 1M / 3M 动量 | 8 / 15 | 1.0 / 0.8 |
| ETF自身 | 距50日线 / 200日线 | 8 / 18 | 0.8 / 1.1 |
| ETF自身 | `σ200` / 日波动率 | 1.5 / 2 | 1.1 / 0.7 |
| ETF自身 | 拥挤度 | 18 | 1.0 |
| 权益环境 | SPY 1M / 3M 动量、距200日线 | 6 / 12 / 12 | 1.1 / 0.8 / 0.8 |
| 权益环境 | QQQ 1M / 3M 动量、距200日线 | 8 / 15 / 15 | 1.2 / 0.9 / 0.9 |
| 波动率 | VIX绝对水平、5日变化、1M变化 | 12 / 18 / 30 | 1.2 / 1.3 / 1.0 |
| 金融条件 | DXY 1M / 3M变化 | 3 / 5 | 1.0 / 0.8 |
| 金融条件 | 10Y收益率水平 / 1M变化 | 8 / 8 | 1.0 / 1.0 |
| 实物资产 | 黄金 1M / 3M变化 | 6 / 10 | 0.6 / 0.5 |
| 实物资产 | 原油 1M / 3M变化 | 8 / 14 | 0.5 / 0.4 |

例如，若当前环境与 `2024-12-03` 的距离为 `0.67`，含义是：按照上述 ETF 趋势、拥挤度、权益、波动率、美元、利率、黄金和原油变量综合衡量，`2024-12-03` 是当前最接近的历史起点之一。该日期之后出现负收益，只是一个需要认真观察的历史路径，不表示当前市场将机械复制当时的回撤。

为避免同一轮行情中的连续周度记录被重复计数，模块会把相邻不超过28天、且阶段总跨度不超过63天的相似样本聚合为一个独立历史阶段，并选取其中距离最低的记录作为阶段代表。路径均值、胜率与分位数优先基于独立阶段代表样本计算。原始日期仍会保留在明细表中，便于复核。

对于负收益或较大回撤的尾部案例，页面会额外展示起点脆弱性和内置历史事件线索。当前尾部案例判定为：未来3M收益为负，或未来3M最大回撤不高于 `-5%`，或未来1M/3M/6M收益全部为负。事件标签只会在相似样本和后续路径已经确定后用于辅助复盘，不参与距离计算，不构成单一因果归因，也不代表当前环境会重复同一路径。

页面还会汇总尾部阶段数量、尾部阶段占比和最接近尾部案例的距离。该指标回答“当前起点与历史脆弱窗口有多接近”，而不是预测未来回撤概率。

实际组合持仓表会额外显示“距年内高点”回撤观察。持仓数量和平均成本来自 Revolut statement；当前价格、前收、当日变化、年内峰值和 FX 参考值来自导入时抓取的 Yahoo 行情。距峰值回撤超过 `5%` 标记为黄色观察，超过 `10%` 标记为红色观察。该分层用于提示趋势、估值和仓位风险复核，不构成机械加减仓建议；后续可进一步结合波动率、SMA200 和基本面事件区分正常回撤与趋势破坏。
